...
export async function addHotel(...args){
    ...
    newHotel.picture = args[7] ? args[7].map(web => helper.checkWebsite(web, true)) : undefined;
    newHotel.rooms = [];
    newHotel.room_types = [];
    newHotel.overall_rating = 0;
    if(args[7] && Array.isArray(args[7])) 
    {
        newHotel.facilities = args[7].map(facility => helper.checkString(facility, "facility", true));
    }
    else if(!args[7])
    {
        newHotel.facilities = [];
    ...
    newHotel.managers = args[8] ? args[8].map(manager => helper.checkId(manager, true)) : undefined;
    newHotel.reviews = [];

    const tempHotel = await Hotel();
    const insertInfo = await tempHotel.insertOne(newHotel);
    if(insertInfo.insertedCount === 0) throw CustomException("Insert hotel failed.", true);
...
export async function updateHotel(...args){
    ...
    updateHotel.picture = args[8] ? args[8].map(web => helper.checkWebsite(web, false)) : undefined;
    updateHotel.rooms = [];
    if(args[8] && Array.isArray(args[8])) 
    {
        updateHotel.facilities = args[8].map(facility => helper.checkString(facility, "facility", false));
    }
    else if(!args[8])
    {
    ...
    updateHotel.managers = args[9] ? args[9].map(manager => helper.checkId(manager, false)) : [];
    updateHotel.roomTypes = args[10] ? args[10].map(roomType => helper.checkString(roomType, false)) : [];
    updateHotel.reviews = args[11] ? args[11].map(reviews => helper.checkString(reviews, false)) : [];

    const tempHotel = await Hotel();
    const updateInfo = await tempHotel.findOneUpdate({_id: hotel_id}, {$set: updateHotel}, {returnDocument: "after"});
    if(!updateInfo) throw CustomException(`Update hotel with id ${hotel_id} failed.`, true);
...
export async function checkRoomAvailability(...args){
    ...
    temp.push(await tempOrder.find({_id: {$in: room_orders.orders}}, {_id: 0, checkin_date: 1, checkout_date: 1, status: 1}));
    if (temp.every(order => order.status === 'canceled' || checkin_date.isAfter(moment(order.checkout_date, 'YYYY/MM/DD')) || checkout_date.isBefore(moment(order.checkin_date, 'YYYY/MM/DD')))){
        return true
    }
